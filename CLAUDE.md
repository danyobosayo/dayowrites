...
- **Food reviews:** photos live in `public/assets/food/<meal-slug>/` (one folder per
  restaurant visit, e.g. `2026-04-14-cafe`); folders were generated by clustering EXIF
  timestamps (>90 min gap = new meal, `exiftool` + resize script at `--max 1600 --quality 75`).
  All words live in `app/food/data.json`, but don't edit it by hand: run `npm run dev`,
  open `/food`, and every card becomes an inline editor (place, area, overall rating, vibe,
  plus name/rating/note for the carousel's current photo). Save posts to the dev-only
  `app/api/food` route, which rewrites `data.json`; commit the JSON when done. Production
  builds render read-only cards and the API answers 404. Types + fs helper are in
  `app/food/meals.ts`; carousel in `app/food/carousel.tsx`; editor in
  `app/food/editable-card.tsx`. Camera clock was on Dallas time until the night of
  Apr 12, verified KST/JST after (receipt + phone-screen timestamps match EXIF).
